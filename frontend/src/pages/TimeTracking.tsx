...
export default function TimeTracking() {
  ...
  const { data: tasks } = useQuery({
    ...
    queryFn: () => user?.id ? tasksAPI.getTasks({ assigned_to: user.id, page: 1, per_page: 100, exclude_status: 'completed' }) : Promise.resolve({ data: [], current_page: 1, last_page: 1 }),
  });
  ...
  useEffect(() => {
    ...
    const handleOffline = () => {
      if (isTrackingRef.current) {
        ...
        toast.warning('Internet connection lost. Tracking continues locally...');
      }
    };
