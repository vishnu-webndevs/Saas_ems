...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($currentUser && !$currentUser->isSuperAdmin() && $currentUser->company) {
            ...
            if ($activePlan && $activePlan->max_users > 0) {
                $currentUsersCount = User::where('company_id', $company->id)->count();
                if ($currentUsersCount >= $activePlan->max_users) {
                    return response()->json([
                        'message' => 'Plan user limit reached. Please upgrade your plan to add more users.',
