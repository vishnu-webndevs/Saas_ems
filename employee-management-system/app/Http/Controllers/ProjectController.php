...
class ProjectController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($currentUser && !$currentUser->isSuperAdmin() && $currentUser->company) {
            ...
            if ($activePlan && $activePlan->max_projects > 0) {
                $currentProjectsCount = Project::where('company_id', $company->id)->count();
                if ($currentProjectsCount >= $activePlan->max_projects) {
                    return response()->json([
                        'message' => 'Plan project limit reached. Please upgrade your plan to add more projects.',
                        'limit' => $activePlan->max_projects,
